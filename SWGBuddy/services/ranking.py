...
class RankingService(Core):
	...
	def _process_message(self, packet):
		...
		elif action == "rank_schematic":
			self._handle_rank_schematic(packet)
	...
	def _handle_rank_schematic(self, packet):
		server_id = packet.get('server_id')
		schematic_data = packet.get('schematic')
		
		if not server_id or not schematic_data: return
		
		sch_id = schematic_data.get('id')
		
		# 1. Update local definitions (Runtime update)
		if server_id not in self.schematic_definitions:
			self.schematic_definitions[server_id] = {}
		
		self.schematic_definitions[server_id][sch_id] = schematic_data
		
		self.info(f"Calculating rankings for new/updated schematic: {sch_id}...")

		# 2. Iterate slots and rank
		# Logic copied/adapted from _handle_recalculate_all but scoped to one schematic
		lookup = self.ingredient_lookup.get(server_id, {})
		slots = schematic_data.get('slots', {})
		...
		self.info(f"Ranking complete for {sch_id}.")
